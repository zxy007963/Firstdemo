...
public class FoodAdapter extends RecyclerView.Adapter<FoodAdapter.MyViewHolder> {
    ...
    public void setList(ArrayList<Steps> list) {
        this.list = list;
    }
    ...
    public void onBindViewHolder(MyViewHolder holder, final int position) {
        Picasso.with(context).load(list.get(position).getImg()).resize(wm.getDefaultDisplay().getWidth(),300).into(holder.low_iv);
        holder.low_low_tv.setText(list.get(position).getStep());
        if (myOnclick!=null){
